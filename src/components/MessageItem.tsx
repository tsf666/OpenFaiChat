...
interface Props {
  ...
  showEdit?: Accessor<boolean>
  onRetry?: () => void
  onCopyAll?: () => void
  onEdit?: () => void
}

export default ({ role, message, showRetry, showEdit, onRetry, onCopyAll, onEdit }: Props) => {
  const roleClass = {
        ...
        <div class="flex items-center">
          {showEdit?.() && onEdit && (
            <div
              ...
              classList={{
                'bg-white/50 dark:bg-white/20 opacity-100 scale-[0.98]': edited(),
                'opacity-70 hover:bg-slate-500/10': !edited()
              }}
            >
              <IconEdit />
              <span>{edited() ? 'Done' : 'Edit'}</span>
            </div>
          )}
